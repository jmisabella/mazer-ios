...
struct OrthoSquareMazeView: View {
    ...
    init(
        ...
        self.columns = Array(repeating: GridItem(.flexible(), spacing: 0), count: cols)
    ...
    var body: some View {
        let totalWidth = cellSize * CGFloat(columns.count)
        let totalHeight = cellSize * CGFloat((cells.map(\.y).max() ?? 0) + 1)
