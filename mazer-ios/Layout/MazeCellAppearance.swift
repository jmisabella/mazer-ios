...
func cellBackgroundColor(
    ...
    if cell.isStart {
        ...
        if totalRows > 1 {
            let lightColor = interpolateColor(from: defaultBackground, to: .white, factor: 0.9) // Very subtle lightening
            let factor = Double(cell.y) / Double(totalRows - 1)
            print("light color: \(lightColor), defaultBackground: \(defaultBackground), factor: \(factor)")
            return interpolateColor(from: lightColor, to: defaultBackground, factor: factor)
        } else {
